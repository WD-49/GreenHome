            ...
            <tr>
                ...
                <td>
                    <div class="d-flex gap-2">
                        <form action="{{ route('admin.account.comment.toggleStatus', $comment->id) }}" method="POST" style="margin: 0;">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-info">Chuyển trạng thái</button>
                        </form>

                        @if (!$comment->deleted_at)
                            <form action="{{ route('admin.account.comment.softDelete', $comment->id) }}" method="POST" style="margin: 0;">
                                @csrf
