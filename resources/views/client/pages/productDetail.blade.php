    ...
    <div class="container">
        <div class="row mb-minus-24" data-aos="fade-up" data-aos-duration="2000" data-aos-delay="600">
                            ...
                            @foreach ($product->productVariants as $variant)
                                <div class="slider-banner-image">
                                    <div class="zoom-image-hover">
                                        <img src="{{ asset('storage/' . ($variant->image ?? 'default.jpg')) }}" alt="{{ $variant->attribute_name }}" class="product-image">
                                    </div>
                                </div>
                            @endforeach
                        </div>
                        <div class="slider slider-nav thumb-image">
                            @foreach ($product->productVariants as $variant)
                                <div class="thumbnail-image">
                                    <div class="thumbImg">
                                        <img src="{{ asset('storage/' . $variant->image) }}" alt="{{ $variant->attribute_name }}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
            ...
            <div class="col-xxl-8 col-xl-7 col-md-6 col-12 mb-24">
                ...
                @php
                    $reviews = $product->comments->whereNotNull('rating');
                ...
                <div class="cr-size-and-weight">
                    <div class="cr-review-star">
                        ...
                        <p>({{ $totalReviews }} lượt đánh giá | {{ $avgRating }}★ | {{ $totalStar }} sao tổng)</p>
                    </div>
                    <div class="list">
                        <ul>
            ...
            <div class="col-12 pt-5">
                <div class="cr-paking-delivery">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="comment-tab" data-bs-toggle="tab" data-bs-target="#comment" type="button" role="tab">Comment</button>
                        </li>
                    </ul>
                                ...
                                <div class="post">
                                    @forelse ($reviews as $review)
                                        <div class="content {{ !$loop->first ? 'mt-30' : '' }}">
                                            <img src="{{ asset('storage/' . ($review->user->avatar ?? 'default-avatar.jpg')) }}" alt="review">
                                            <div class="details">
                            ...
                            <div class="cr-tab-content-from">
                                <div class="post">
                                    ...
                                    @forelse ($comments as $comment)
                                        <div class="content {{ !$loop->first ? 'mt-30' : '' }}">
                                            <img src="{{ asset('storage/' . ($comment->user->avatar ?? 'default-avatar.jpg')) }}" alt="comment">
                                            <div class="details">
                    ...
                    <div class="cr-product-card">
                        <div class="cr-product-image">
                            <div class="cr-image-inner zoom-image-hover">
                                <img src="{{ asset('storage/' . ($item->productVariants->first()->image ?? 'default.jpg')) }}" alt="{{ $item->name }}">
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('.variant-option').forEach(function(li) {
            li.addEventListener('click', function() {
                document.querySelectorAll('.variant-option').forEach(e => e.classList.remove('active-color'));
                this.classList.add('active-color');
                let gia = Number(this.dataset.price);
                document.getElementById('variant-price').textContent = gia.toLocaleString('vi-VN') + '₫';
            });
        });
    });
</script>
